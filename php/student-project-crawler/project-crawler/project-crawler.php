<?php
require("config.php");
if(!defined("projectPath")) die("config file is missing");
set_time_limit(60);
...
if ( !defined('ABSPATH') ) {
	...
	require_once(projectPath.'/wp-load.php');
...
{
	global $messages;
	$mypath = (string)WP_PLUGIN_DIR.'\vanarts-project-crawler\\'.logfile;
	
	$content = "Date: ".date("d-m-Y G:i")."\r\n";
...
function populate_wordpress($work)
{
	...
	{//create post
		...
		if($projectImages !== false)
		{
			...
			{
				$content.='<a href="'.$image['url'].'" rel="'.sanitize_title_with_dashes($work['name']).'"><img class="alignleft size-medium wp-image-'.$image['thumbWidth'].'" title="'.$work['name'].'" src="'.$wp_upload_dir['url']."/".$image['thumbName'].'" alt="" width="'.$image['thumbWidth'].'" height="'.$image['thumbHeight'].'" /></a>&nbsp;';
				if($x==3)
				{
		...
		if($counter >0)
		{
			...
			foreach($data['images'] as $key => $image)
			{
				...
				if($_SERVER['DOCUMENT_ROOT']!="")
					$sourcePath = preg_replace($pattern,$_SERVER['DOCUMENT_ROOT'],$sourceFull);
				else
					$sourcePath = preg_replace($pattern,serverPath,$sourceFull);
				...
				if(copy($sourcePath,$targetFull))
				{
					...
					$picture->save(projectPath."/".$med_filefullname,$picture->image_type,70);
					...
					$picture->save(projectPath."/".$thumb_filefullname,$picture->image_type,70);
					...
					$picture->save(projectPath."/".$thumb_recent_filefullname,$picture->image_type,70);
					...
					$picture->save(projectPath."/".$small_filefullname,$picture->image_type,70);
				...
				{
					array_push($messages,"Copy error: filename ".$sourcePath);
				}
...
$time_total2 = time();
array_push($messages, "All process took total ".($time_total2-$time_total1)." seconds");
log_result();
global $wpdb;
$wpdb->flush();
